...
describe("Part 1", () => {
  it("should be a valid program", async () => {
    expect(
      await compute("1101,100,-1,4,0", [], { mode: PROGRAM_MODES.MEMORY })
  ...
  it("should do multiplication and store the result in the last value", async () => {
    ...
    expect(
      await compute("1002,4,3,4,33", [], { mode: PROGRAM_MODES.MEMORY })
  ...
  it("should produce the correct value for the input data", async () => {
    ...
    expect(await compute(data, input)).toMatchSnapshot();
  });
});
...
describe("Part 2", () => {
  it("should be a valid jump test (using position mode)", async () => {
    expect(
      await compute("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", [0])
    ).toEqual([0]);
    expect(
      await compute("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", [1])
    ).toEqual([1]);
  });

  it("should be a valid jump test (using immediate mode)", async () => {
    expect(await compute("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", [0])).toEqual([
      0
    ]);
    expect(await compute("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", [1])).toEqual([
      1
    ]);
  });
