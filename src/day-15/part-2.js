...
module.exports = async function oxygenSystem(program) {
  ...
  function tryToMove() {
    ...
    computer.input(
      move(neighbours.find(({ movement }) => canMove(state, movement)).movement)
    );
  }
...
function visualize(state) {
  ...
  const visual_board = range(height).map(() =>
    range(width).map(() => DRAW.UNKNOWN)
  );
  ...
  // Set the droid
  visual_board[state.droid.y + TOP_OFFSET][state.droid.x + LEFT_OFFSET] =
    DRAW.DROID;
  ...
  // Draw
  console.log(compactTable(visual_board));
}
